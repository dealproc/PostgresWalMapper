namespace PGWalMapper {
    ...
    internal class InsertOperationHandler<TEvent> : OperationHandler {
        ...
        public override async bool Handle(ReplicationMessage msg) {
            ...
            foreach (var definition in inserted.Relation.Columns) {
                dict.Add(definition.ColumnName, await enumerator.Current.Get());
                await enumerator.MoveNextAsync();
            }

            object obj;
            
            if (_columnMappings.Any(cm => cm.SetOnConstructor)) {
