    ...
    public class Example {
        ...
        public void Start() {
            ...
            _listener = new WalConfigurationBuilder("Host=127.0.0.1;Port=5432;Database=postgres;username=postgres;password=mysecretpassword;", _loggerFactory.CreateLogger<WalListener>())
                ...
                .Column("code").ToProperty(prop => prop.Code).AsConstructorArgument()
                .Column("title").ToProperty(prop => prop.Title).AsConstructorArgument()
                .Column("did").ToProperty(prop => prop.Did).AsConstructorArgument()
                .Column("date_prod").ToProperty(prop => prop.DateProduced).AsConstructorArgument()
                .Column("kind").ToProperty(prop => prop.Kind).AsConstructorArgument()
                ...
                .Column("did").ToProperty(prop => prop.Did).AsConstructorArgument()
                .Column("name").ToProperty(prop=> prop.Name).AsConstructorArgument()
                .OnInsert(o=> _logger.LogInformation($"Insert: {JsonSerializer.Serialize(o, options)}"))
                .OnUpdate(o => _logger.LogWarning($"Update: {JsonSerializer.Serialize(o, options)}"))
                .OnDelete(o => _logger.LogCritical($"Delete: {JsonSerializer.Serialize(o, options)}"))
